(function(){
  ...
  function init(){const grid=document.querySelector('#exercise-grid'),box=document.querySelector('.categories');if(!grid)return;const observer=new MutationObserver(sync);observer.observe(grid,{childList:true,subtree:true});if(box)observer.observe(box,{childList:true,subtree:true});cards();updateChip()}
