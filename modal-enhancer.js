(function(){
  ...
  function enhance(){
    ...
    dataPromise.then(all=>{const e=all.find(x=>String(x.id)===String(id));if(!e)return;styles();
      ...
      if(existing&&(e.primary_muscles||[]).length){const box=document.createElement('div');box.className='hs-primary';box.innerHTML='<div class="hs-primary-title">Muscles ciblés en priorité</div><div class="hs-primary-tags">'+e.primary_muscles.map(m=>'<span class="hs-primary-tag">'+label(m)+'</span>').join('')+'</div>'+(e.secondary_muscles||[]).length?'<div class="hs-secondary">Secondaires : '+e.secondary_muscles.map(label).join(' · ')+'</div>':'';existing.insertAdjacentElement('afterend',box)}
