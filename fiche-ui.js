(function(){
  ...
  function enhance(){
    ...
    if(!fiche.querySelector('.hs-fiche-kicker')){
      const kicker=document.createElement('div');kicker.className='hs-fiche-kicker';kicker.textContent='FICHE EXERCICE';title.parentNode.insertBefore(kicker,title);
    }
    if(!fiche.querySelector('.hs-fiche-divider')){
      const divider=document.createElement('div');divider.className='hs-fiche-divider';const firstSection=fiche.querySelector('.hs-meta');if(firstSection)firstSection.parentNode.insertBefore(divider,firstSection);
    }
    if(!fiche.querySelector('.hs-fiche-close')){
      const btn=document.createElement('button');btn.type='button';btn.className='hs-fiche-close';btn.setAttribute('aria-label','Fermer la fiche');btn.textContent='Fermer';btn.addEventListener('click',()=>{const close=document.querySelector('#close-modal');if(close)close.click();else if(typeof modal.close==='function')modal.close()});fiche.appendChild(btn);
    }
    if(!document.querySelector('#hs-fiche-ui-style')){
      const s=document.createElement('style');s.id='hs-fiche-ui-style';s.textContent=`
        ...
        #exercise-modal .hs-primary{margin-top:18px}
        ...
        /* Une seule illustration dans la fiche : plus lisible sur iPhone. */
        #exercise-modal .modal-media.media-pair{display:block!important;width:100%!important}
        #exercise-modal .modal-media.media-pair .media-a{display:block!important;width:100%!important;height:auto!important}
        #exercise-modal .modal-media.media-pair .media-b{display:none!important}
